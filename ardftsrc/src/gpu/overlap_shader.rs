...
pub(crate) fn glsl_scalar_name(scalar: ScalarType) -> Result<&'static str, GpuError> {
    match scalar {
        ...
        _ => Err(GpuError::PlanCreationFailed(
            "overlap-add shader only supports f32/f64".to_string(),
        )),
    }
}
...
"
    ...
    glsl.push_str(&format!(
        "layout(set = 0, binding = 1, std430) buffer OutBuf {{ {float_ty} out_buf[]; }};\n"
    ));
    glsl.push_str(&format!(
        "layout(set = 0, binding = 2, std430) buffer OverlapBuf {{ {float_ty} overlap[]; }};\n"
    ));
    glsl.push_str(&format!(
        r"
void main() {{
...
pub(crate) fn compile_overlap_shader(
    scalar: ScalarType,
    mode: OverlapMode,
    ifft_stride: usize,
    output_chunk_frames: usize,
