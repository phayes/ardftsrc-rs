...
mod tests {
    ...
    fn input_chunk_frames(resampler: &ChunkInterleavedResampler<f32>) -> usize {
        resampler.input_chunk_size() / resampler.config().channels
    }

    fn output_chunk_frames(resampler: &ChunkInterleavedResampler<f32>) -> usize {
    ...
    fn resample_stream_with_context(
        ...
        let mut resampler = ChunkInterleavedResampler::new(config).unwrap();
        if let Some(pre) = pre {
    ...
    fn silence_stays_silent() {
        let mut resampler = ChunkInterleavedResampler::new(mono_config(48_000, 48_000)).unwrap();
    ...
    fn impulse_output_is_finite() {
        let mut resampler = ChunkInterleavedResampler::new(mono_config(44_100, 48_000)).unwrap();
    ...
    fn expected_output_size_matches_frame_count_conversion() {
        let resampler = ChunkInterleavedResampler::<f32>::new(stereo_config(44_100, 48_000)).unwrap();
    ...
    fn stereo_wrapper_matches_channel_core_outputs() {
        ...
        let mut wrapper = ChunkInterleavedResampler::<f32>::new(config.clone()).unwrap();
    ...
    fn pre_and_post_reject_wrong_channel_count() {
        let mut resampler = ChunkInterleavedResampler::<f32>::new(stereo_config(44_100, 48_000)).unwrap();
    ...
    fn pre_context_changes_first_chunk_output() {
        let mut with_zero_pre = ChunkInterleavedResampler::new(mono_config(44_100, 48_000)).unwrap();
        let mut with_one_pre = ChunkInterleavedResampler::new(mono_config(44_100, 48_000)).unwrap();
    ...
    fn post_context_changes_flush_output() {
        let mut with_zero_post = ChunkInterleavedResampler::new(mono_config(44_100, 48_000)).unwrap();
        let mut with_one_post = ChunkInterleavedResampler::new(mono_config(44_100, 48_000)).unwrap();
    ...
    fn streaming_and_offline_paths_match() {
        ...
        let mut streaming = ChunkInterleavedResampler::new(config).unwrap();
    ...
    fn split_resampling_with_pre_post_matches_full_stream() {
        ...
        let mut full_resampler = ChunkInterleavedResampler::new(config.clone()).unwrap();
    ...
    fn process_all_resets_state_between_calls() {
        let config = mono_config(44_100, 48_000);
        let mut reused = ChunkInterleavedResampler::new(config.clone()).unwrap();
        let mut reference = ChunkInterleavedResampler::new(config).unwrap();
