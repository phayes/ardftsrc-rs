...
const INPUT_BUFFER_SIZE_MULTIPLIER: usize = 8;
    ...
    T: Float + FftNum,
{
    ...
    // Initial input sample delay to allow the resampler to prime the FFT.
    ...
    initial_input_sample_delay: usize,

    // Initial output sample delay to allow the resampler to prime the output ring buffer.
    // This can change as the sample-rate ratio and channel count changes across spans (unlikely in first few ms, but still possible)
    initial_output_sample_delay: usize,
    ...
    T: Float + FftNum,
{
    ...
    pub fn new(config: Config) -> Result<Self, Error> {
        ...
        let mut instance = Self {
            ...
            config,
            stop_thread,
            initial_input_sample_delay: 0,
            initial_output_sample_delay: 0,
        ...
        instance.set_initial_sample_delays();
    ...
    fn output_chunk_samples(&self) -> usize {
        let input_sample_rate = self.span_format_in.sample_rate as usize;
        let output_sample_rate = self.config.output_sample_rate;
        let channels = self.span_format_in.channels as usize;
        ...
        let input_chunk_frames = input_sample_rate / common_divisor;
        let mut output_chunk_frames = output_sample_rate / common_divisor;
        let denominator = input_chunk_frames.min(output_chunk_frames).max(1);
        let factor = self.config.quality.div_ceil(denominator).next_multiple_of(2);
        output_chunk_frames *= factor;

        output_chunk_frames * channels
    }
    ...
    // Set the initial sample delays to multiple chunks on each side of the resampler.
    fn set_initial_sample_delays(&mut self) {
        self.initial_input_sample_delay = self.input_chunk_samples() * INITIAL_SAMPLE_DELAY_CHUNKS;
        self.initial_output_sample_delay = self.output_chunk_samples() * INITIAL_SAMPLE_DELAY_CHUNKS;
    }

    /// Get the initial input sample delay.
    #[inline]
    #[must_use]
    pub fn initial_input_sample_delay(&self) -> usize {
        self.initial_input_sample_delay
    }

    /// Get the initial output sample delay.
    #[inline]
    #[must_use]
    pub fn initial_output_sample_delay(&self) -> usize {
        self.initial_output_sample_delay
    }

    pub fn initial_sample_delay_duration(&self) -> std::time::Duration {
        let channels: usize = usize::from(self.span_format_out.channels).max(1);
        let output_sample_rate_hz = self.span_format_out.sample_rate;

        // `initial_output_sample_delay` is tracked in interleaved output samples.
        // Convert to output frames before converting to wall-clock duration.
        let output_frames = self.initial_output_sample_delay / channels;
        std::time::Duration::from_secs_f64(output_frames as f64 / f64::from(output_sample_rate_hz))
    }
    ...
    fn fill_local_read_buffer(&mut self) {
        ...
        if self.output_samples_pending() < self.output_chunk_samples() / 2 {
            tracing::trace!("ardftsrc-rs: output ring buffer has less than half a buffer-worth of samples");
        }
    }

    #[inline]
    #[must_use]
    pub fn output_samples_pending(&self) -> usize {
        self.out_consumer.slots() + self.local_read_buffer.len()
    }
    ...
    pub fn read_sample(&mut self) -> Option<T> {
        ...
        if self.total_input_samples_written < self.initial_input_sample_delay {
    ...
    pub fn new_span(&mut self, input_sample_rate: usize, channels: usize) {
        ...
        self.set_initial_sample_delays();
