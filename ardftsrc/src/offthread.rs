...
pub struct OffThreadStreamingResampler<T = f64>
where
    T: Float + FftNum,
{
...
impl<T> OffThreadStreamingResampler<T>
where
    T: Float + FftNum,
{
...
mod tests {
    ...
    fn input_chunk_frames(resampler: &OffThreadStreamingResampler<f32>) -> usize {
    ...
    fn resample_stream_with_sample_api(
        ...
        let mut resampler = OffThreadStreamingResampler::new(config).unwrap();
    ...
    fn drain_stream(resampler: &mut OffThreadStreamingResampler<f32>, read_block_size: usize) -> Vec<f32> {
        let mut output = Vec::new();
        let mut read_buffer = vec![0.0; read_block_size.max(1)];
        loop {
            let written = resampler.read_samples(&mut read_buffer);
    ...
    fn new_span_preserves_output_rate_and_quality_settings() {
        ...
        let mut resampler = OffThreadStreamingResampler::<f32>::new(config).unwrap();
    ...
    fn finalize_samples_rejects_dangling_partial_frame_for_active_span() {
        let mut resampler = OffThreadStreamingResampler::new(mono_config(44_100, 48_000)).unwrap();
        resampler.new_span(44_100, 2).unwrap();
        resampler.write_samples(&[0.0]).unwrap();
    ...
    fn write_samples_and_read_samples_match_chunk_output() {
        ...
        let mut sample_resampler = OffThreadStreamingResampler::new(config).unwrap();
    ...
    fn sample_api_finalize_matches_process_all_total_output() {
        ...
        let driver = OffThreadStreamingResampler::<f32>::new(config.clone()).unwrap();
    ...
    fn finalize_samples_read_until_zero_drains_stream() {
        let config = mono_config(44_100, 48_000);
        let mut offline = InterleavedResampler::new(config.clone()).unwrap();
        let mut stream = OffThreadStreamingResampler::new(config).unwrap();
