...
use super::StrengthReducedU64;
use super::long_multiplication;
...
#[inline]
fn divide_128_by_64_preshifted_reduced(
    numerator_hi: u64,
    numerator_lo: u64,
...
pub(crate) fn divide_256_max_by_128(divisor: u128) -> (u128, u128) {
    ...
    let divisor_chunks = [divisor_shifted as u64, (divisor_shifted >> 64) as u64];
    ...
    for quotient_idx in (0..num_quotient_chunks).rev() {
        ...
        // scope for borrow checker
        {
            ...
            let numerator_hi = if numerator_slice.len() - numerator_start_idx > 1 {
                numerator_slice[numerator_start_idx + 1]
            } else {
                0
            };
